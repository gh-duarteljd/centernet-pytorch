...
class kp_module(nn.Module):
     def __init__(self, n, dims, modules, **kwargs):
         ...
         # Perform two residual convolutions on the input feature layer to facilitate fusion with the subsequent layer
         self.up1 = make_layer(
         ...
         # Build the next layer of the U-shaped structure
         if self.n > 1 :
             self.low2 = kp_module(
         ...
         # Perform residual convolution on the layer fed back from the next layer of the U-shaped structure
         self.low3 = make_layer_revr(
             3, next_dim, curr_dim, curr_mod, **kwargs
         )
         # Upsample the layer fed back from the next layer of the U-shaped structure
