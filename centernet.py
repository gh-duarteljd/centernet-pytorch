...
#-----------------------------------------------#
# Use your own trained model to predict and need to modify 3 parameters
# model_path, classes_path and backbone
# All need to be modified!
# If there is a shape mismatch, be sure to pay attention
...
class CenterNet(object):
     _defaults = {
         ...
         #
         # After training, there are multiple weight files in the logs folder, just select the one with lower loss in the validation set.
         # The lower loss of the verification set does not mean that the mAP is higher, it only means that the weight has better generalization performance on the verification set.
         ...
         "nms_iou" : 0.3,
         #------------------------------------------------- --------------------------#
         # Whether to perform non-maximum suppression, you can choose according to the detection effect
     ...
     def detect_image(self, image, crop = False, count = False):
         ...
         #------------------------------------------------- ----------#
         # Image preprocessing, normalization. The shape of the obtained photo is [1, 512, 512, 3]
         #------------------------------------------------- ----------#
         ...
         with torch.no_grad():
             ...
             #------------------------------------------------- ----------#
             # Use the predicted result to decode
             #------------------------------------------------- ----------#
             outputs = decode_bbox(outputs[0], outputs[1], outputs[2], self.confidence, self.cuda)

             #------------------------------------------------- ------#
             # For the centernet network, it is very important to establish the center.
             # For large targets, there will be a lot of local information.
             # At this time, for the same large target, the center point is more difficult to determine.
             # Using the non-maximum suppression method of max pooling cannot remove the local box
             # So I still wrote another piece of code for non-maximum suppression of the frame
             # In the actual test, when hourglass is the backbone network, there is little difference with or without additional nms, but resnet has a big difference.
             #------------------------------------------------- ------#
     ...
     def get_FPS(self, image, test_interval):
         ...
         for _ in range(test_interval):
             with torch.no_grad():
                 ...
                 #------------------------------------------------- ------#
                 # For the centernet network, it is very important to establish the center.
                 # For large targets, there will be a lot of local information.# At this time, for the same large target, the center point is more difficult to determine.
                 # Using the non-maximum suppression method of max pooling cannot remove the local box
                 # So I still wrote another piece of code for non-maximum suppression of the frame
                 # In the actual test, when hourglass is the backbone network, there is little difference with or without additional nms, but resnet has a big difference.
                 #------------------------------------------------- ------#
